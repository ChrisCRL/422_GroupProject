 ...
 * Output: 2 tables showing gender, year in school, majors, and payment type
 *         feature variables when predicted based on student expense categories.
 *         The tables show results for linear regression, KNN, and SVM models.
 *         Count plots are also created based on the categorical data within
 *         the dataframe.
"""
import pandas as pd
import numpy as np
...
def count_plot(category, df):
    ...
    # - plotting data -
    plot = sns.countplot(x=category, data=df, palette='mako', hue=category, legend=False)
...
# creating a dataframe with encoded data and converts data into int (originally converts to float for some reason)
...
# - dropping Student since it doesn't provide any use
# - dropping categorical data to replace with encoded data
